...
def _normalisasi_tag_value(value):
    if not value:
        return None
...
def _ambil_tag(tags, *nama_tags):
    if not tags:
        return None

    for nama in nama_tags:
        value = _normalisasi_tag_value(tags.get(nama))
        if value:
            return value
    ...
    for key, value in tags.items():
        if str(key).lower() in nama_lower:
            value = _normalisasi_tag_value(value)
            if value:
                return value
...
def get_audio_metadata(file_path):
    ...
    def ambil(easy_names, raw_names):
        return (
            _ambil_tag(easy_tags, *easy_names)
            or _ambil_tag(raw_tags, *raw_names)
            or "Unknown"
        )
