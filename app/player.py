...
def shuffle_queue(guild_id):
    if guild_id not in state.queues or not state.queues[guild_id]:
        return
    temp = list(state.queues[guild_id])
    sf(temp)
    state.queues[guild_id] = deque(temp)
...
async def play_next(guild_id, voice_client):
    ...
    while state.play_queue[guild_id]:
        file_rel_path = state.play_queue[guild_id].popleft()
        try:
            if file_rel_path in state.queue_asli.get(guild_id, []):
                state.queue_asli[guild_id].remove(file_rel_path)
        except ValueError:
            pass
