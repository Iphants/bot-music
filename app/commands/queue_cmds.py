...
def setup(bot: commands.Bot) -> None:
    ...
    async def shuffle(ctx):
        ...
        async with player.kunci_lagu(guild_id):
            player.ensure_deques(guild_id)
            if (
                not state.queue_asli.get(guild_id)
            ...
            return await ctx.send(
                "gw kocok yaa antriannya, gausah nyesel klo muter yg aneh"
            )
    ...
    async def remove(ctx, *, target):
        ...
        async with player.kunci_lagu(guild_id):
            ...
            if len(exact_matches) > 1:
                lines = [
                    f"{i + 1}. {item_name(item)}"
                    for i, item in enumerate(exact_matches)
                ]
                return await ctx.send(
                    "yg mana jir, ada banyak, hapus make angka:\n" + "\n".join(lines)
                )
            ...
            return await ctx.send(
                "ga ketemu antriannya, pmo mulu sih jadi lupa antriannya sendiri"
            )
